"""RQ5: BPNN (MLP, tanh activation, GLCM + HOG features) vs CNN proposed model.
...
Saves results and comparison table to results/rq5_results.json.
"""
...
def main():
    ...
    log(f"\n{'='*80}")
    log(f"RQ5: BPNN HYPERPARAMETER SEARCH (GridSearchCV, 5-fold, scoring=AUC)")
    log(f"{'='*80}")
    ...
    log(f"\n{'='*80}")
    log(f"RQ5: BPNN 5-FOLD CV — Youden threshold search (best params)")
    log(f"{'='*80}")
    ...
    log(f"\n{'='*80}")
    log(f"RQ5: BPNN final training on full train set  ({avg_iter} iterations)")
    log(f"{'='*80}")
    ...
    log(f"\n{'='*80}")
    log(f"RQ5: BPNN TEST RESULTS  (threshold={mean_youden:.4f})")
    log(f"{'='*80}")
    for k, v in bpnn_metrics.items():
        log(f"  {k:<14}: {v:.4f}")

    # ── Comparison with CNN (RQ3) ──────────────────────────────────────────────
    rq3_path      = os.path.join(CONFIG['results_dir'], 'rq3_results.json')
    rq3_probs_path = os.path.join(CONFIG['results_dir'], 'rq3_test_probs.npy')
