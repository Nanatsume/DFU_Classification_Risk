...
def test_save_crf_mints_when_pid_is_none(tmp_db):
    ...
    pid = tmp_db.save_crf(None, "t", fields={}, derived={}, schema_version="1.0")
    assert pid == "P0001"
    assert tmp_db.get_crf("P0001") is not None
    assert tmp_db.save_crf(None, "t", fields={}, derived={}, schema_version="1.0") == "P0002"


def test_save_crf_returns_the_pid_it_was_given(tmp_db):
    assert tmp_db.save_crf("P0042", "t", fields={}, derived={},
...
def test_deleted_form_does_not_free_its_id(tmp_db):
    ...
    tmp_db.save_crf(None, "t", fields={}, derived={}, schema_version="1.0")
    tmp_db.delete_crf("P0001")
    assert tmp_db.save_crf(None, "t", fields={}, derived={},
                           schema_version="1.0") == "P0002"


def test_save_crf_overwrites_on_conflict(tmp_db):
    tmp_db.save_crf("P0001", "t1", fields={"note": "first"}, derived={}, schema_version="1.0")
    tmp_db.save_crf("P0001", "t2", fields={"note": "second"}, derived={}, schema_version="1.0")
...
def test_delete_crf_returns_true_when_present(tmp_db):
    tmp_db.save_crf("P0001", "t", fields={}, derived={}, schema_version="1.0")
...
def test_list_cases_with_status_only_includes_cases_with_a_form(tmp_db):
    ...
    tmp_db.save_crf("P0002", "t", fields={}, derived={"L": {"category": 1}, "R": {}}, schema_version="1.0")
...
def test_committed_max_vs_crf_max_are_independent_counters(tmp_db):
    tmp_db.save_crf("P0005", "t", fields={}, derived={}, schema_version="1.0")
    assert tmp_db.crf_max() == 5
    assert tmp_db.committed_max() == 0  # no commit yet
    tmp_db.save_commit("P0003", "complete", "t")
...
def test_list_commits_reconstructs_manifest_shape(tmp_db):
    ...
    tmp_db.save_commit("P0001", "complete", "2026-01-01T00:00:00+07:00")
