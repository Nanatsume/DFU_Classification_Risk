"""RQ3: Final performance evaluation of the proposed model.
...
def main():
    ...
    # ── Load prerequisites ────────────────────────────────────────────────────
    for fname, label in [('rq1_results.json', 'RQ1'), ('rq2_results.json', 'RQ2')]:
    ...
    with open(os.path.join(CONFIG['results_dir'], 'rq1_results.json')) as f:
        rq1 = json.load(f)
    with open(os.path.join(CONFIG['results_dir'], 'rq2_results.json')) as f:
        rq2 = json.load(f)
    ...
    log(f"\n{'='*80}")
    log(f"RQ3: TEST SET EVALUATION — {best_model}  threshold={threshold:.4f}")
    log(f"{'='*80}")
