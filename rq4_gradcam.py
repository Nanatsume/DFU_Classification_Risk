"""RQ4: Grad-CAM, Grad-CAM++, and Eigen-CAM visualisations on the proposed model.

Applies all three methods to sample images from the test set and saves
side-by-side heatmap overlays to results/rq4_gradcam/.
...
#
# 4. Result: hit rate per CAM method saved to results/rq4_results.json
...
def run_pointing_game(feat_model, clf_model, all_images, image_names,
    ...
    log(f"\n{'='*80}")
    log(f"RQ4: POINTING GAME RESULTS  "
...
def main():
    ...
    if not os.path.exists(rq3_path):
        log("❌ rq3_results.json not found. Run rq3_final_evaluation.py first.")
        return
    ...
    out_dir = os.path.join(CONFIG['results_dir'], 'rq4_gradcam')
    os.makedirs(out_dir, exist_ok=True)

    log(f"\n{'='*80}")
    log(f"RQ4: CAM VISUALISATIONS — {best_model}  (final retrained model)")
    log(f"{'='*80}")
    ...
    rq4_result = {'best_model': best_model, 'model': 'final_retrain'}
